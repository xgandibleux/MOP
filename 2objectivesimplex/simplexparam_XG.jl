...
#  export parametric_simplex_method
  ...
  function print_tableau(t::SimplexTableau)
    ...
    for j in 1:length(t.alpha0_f1)
      Printf.@printf("%6.2f ", t.alpha0_f1[j])
    end
    Printf.@printf("| %6.2f\n", t.a00_f1)
  ...
  function print_tableau2(t::SimplexTableau)
    ...
    for j in 1:length(t.alpha0_f2)
      Printf.@printf("%6.2f ", t.alpha0_f2[j])
    end
    Printf.@printf("| %6.2f\n", t.a00_f2)
  ...
  function pivoting_biobj!(t::SimplexTableau)
    ...
    # Updating I
    t.I[ findall(t.I.==t.I[exiting]) ] .= entering
  ...
  function pivot_point_biobj(tableau::SimplexTableau)
    ...
    println(lambda)
    IndlambdaPrim= argmax(lambda).I[2]
    entering = Jc[IndlambdaPrim]
    println(entering)

    # variable sortante
    pos_idx = findall( tableau.Y[:, entering] .> 0 )
    if length(pos_idx) == 0
      error("Unbounded")
    end
    exiting = pos_idx[ argmin( tableau.xB[pos_idx] ./ tableau.Y[pos_idx, entering] ) ]
  ...
  function initialize(c, T, d)
    ...
    Y = inv(B) * T
    c_B = c[I]
    a00_f1 = LinearAlgebra.dot(c_B, xB)
    a00_f2 = 0.0
  ...
  function isOptimal(tableau)
    ...
    return findfirst( tableau.alpha0_f1 .> 0 ) == nothing
  ...
  function isOptimal_biobj(tableau)
    ...
    return findfirst( tableau.alpha0_f2 .> 0 ) == nothing
  ...
  function parametric_simplex_method(c1, c2, T, d)
    ...
    B = T[:,I]
    c2_B = c2[I]
    tableau.a00_f2 = LinearAlgebra.dot(c2_B, tableau.xB)
