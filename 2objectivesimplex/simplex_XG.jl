...
module SimplexMethod
  ...
  function pivot_point(t::SimplexTableau)
    ...
    entering = findfirst(t.alpha0 .> 0).I[2]
    if entering == nothing
      error("Optimal")
    end
    ...
    exiting = pos_idx[ argmin( t.xB[pos_idx] ./ t.Y[pos_idx, entering] ) ]
  ...
  function isOptimal(tableau)
    ...
    return findfirst( tableau.alpha0 .> 0 ) == nothing
